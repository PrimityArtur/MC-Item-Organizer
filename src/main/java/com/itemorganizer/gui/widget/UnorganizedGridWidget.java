...
public class UnorganizedGridWidget implements Drawable, Element, Selectable {
    ...
    // double right click tracking to block item
    ...
    public boolean mouseClicked(Click click, boolean bl) {
        ...
        // double right click: block item and move to blocked list
        if (click.button() == 1 && hoveredIndex >= 0 && !hoveredStack.isEmpty()) {
            List<String> items = viewModel.getUnorganizedItems();
            ...
            if (isMouseOver(click.x(), click.y())) {
                ...
                if (relX >= 0 && relY >= 0) {
                    ...
                    if (col < cols) {
                        int index = row * cols + col;
                        List<String> items = viewModel.getUnorganizedItems();
                        if (index >= 0 && index < items.size() && index != lastShiftIndex) {
                            lastShiftIndex = index;
                            ItemStack stack = getItemStackFromId(items.get(index));
                            if (!stack.isEmpty()) {
